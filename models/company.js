...
class Company {
  ...
  static async create({ handle, name, description, numEmployees, logoUrl }) {
    const duplicateCheck = await db.query(
      `SELECT handle FROM companies WHERE handle = $1`, [handle]
    );
  ...
  /** Find all companies.
   *
   * Accepts: { name, minEmployees, maxEmployees }
   * Returns [{ handle, name, description, numEmployees, logoUrl }, ...]
  ...
  /** Given a company handle, return data about company including jobs.
   *
  ...
  static async update(handle, data) {
    ...
    const querySql = `UPDATE companies SET ${setCols} WHERE handle = ${handleVarIdx} RETURNING handle, name, description, num_employees AS "numEmployees", logo_url AS "logoUrl"`;
  ...
  static async remove(handle) {
    const result = await db.query(
      `DELETE FROM companies WHERE handle = $1 RETURNING handle`, [handle]
    );
    const company = result.rows[0];
